...
import { FileText, PlusCircleIcon, RefreshCw } from "lucide-react";
import { Link } from "react-router-dom";
import useAdminForm from "../../../hooks/useAdminForm";

const ArticleHeader = ({ windowWidth, onRefresh }) => {
  const { t } = useAdminForm();

  return (
    <div className="d-flex flex-wrap justify-content-between mb-3">
      <h4 className="h5 d-flex align-items-center mb-3 mb-sm-0">
        <FileText size={windowWidth < 576 ? 20 : 28} className="admin-header-icon" />
        {t("admin.articles")}
      </h4>
